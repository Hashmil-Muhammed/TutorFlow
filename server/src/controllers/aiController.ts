...
export const generateSessionReview = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!session.notes || session.notes.trim() === '') {
      return res.status(400).json({ error: 'No notes available to review.' });
    }
    ...
    const prompt = `
      ...
      Based on these notes, generate a session review and homework.
      Output MUST be valid JSON in this exact format, with no markdown formatting around it:
      {
        "summary": "Short paragraph summarizing how the session went based on notes.",
